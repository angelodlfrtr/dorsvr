...
func (p *H264VideoStreamParser) parse() (uint, error) {
	log.Printf("[H264VideoStreamParser] is's parsing h264 file video stream.")
	...
	if !p.haveSeenFirstStartCode {
		...
		for {
			if first4Bytes, err = p.test4Bytes(); err != nil {
				log.Printf("failed to test of start of stream: %s", err.Error())
				return 0, err
			}
			...
			if _, err = p.get1Byte(); err != nil {
				log.Printf("failed to check first 4 bytes: %s", err.Error())
				return 0, err
			}
		...
		if err = p.skipBytes(4); err != nil {
			log.Printf("skip this initial code: %s", err.Error())
			return 0, err
		}
	...
	if p.haveSeenEOF {
		...
		if next4Bytes, err = p.test4Bytes(); err != nil {
			log.Printf("failed to test next 4 bytes: %s", err.Error())
			return 0, err
		}
		//log.Printf("progress next 4 bytes: %d", next4Bytes)
		...
		for next4Bytes != 0x00000001 && (next4Bytes&0xFFFFFF00) != 0x00000100 {
			...
			if err = p.skipBytes(skipBytes); err != nil {
				log.Printf("failed to skip 1 or 4 bytes: %s", err.Error())
				return 0, err
			}

			p.setParseState() // ensures forward progress
			if next4Bytes, err = p.test4Bytes(); err != nil {
				log.Printf("failed to test 4 bytes(), ensures forward progress: %s", err.Error())
				return 0, err
			}
		}
		...
		if err = p.skipBytes(skipBytes); err != nil {
			log.Printf("failed to skip 3 or 4 bytes: %s", err.Error())
			return 0, err
		}
	}
	...
	case 7: // Sequence parameter set
		...
		if spsData.timeScale > 0 && spsData.numUnitsInTick > 0 {
			...
			log.Printf("Get the frameRate(%d) from Sequence parameter set", p.UsingSource().frameRate)
		} else {
		}
	...
	log.Printf("\tPresentation Time: %d.%06d", p.UsingSource().presentationTime.Sec,
	...
	if p.haveSeenEOF {
		...
		if isVCL {
			...
			if err = p.testBytes(firstByteOfNextNALUnit, 1); err != nil {
				log.Printf("failed to test bytes(firstByteOfNextNALUnit): %s", err.Error())
				return 0, err
			}
...
func (p *H264VideoStreamParser) analyzeSliceHeader(start, end []byte, nalUnitType uint) *sliceHeader {
	...
	log.Printf("firstMBInSlice: %d, sliceType: %d", firstMBInSlice, sliceType)
...
func (p *H264VideoStreamParser) analyzeSPSData() *seqParameterSet {
	...
	log.Printf("profileIdc:%d", profileIdc)
	constraintSetNFlag := bv.getBits(8) // also "reserved_zero_2bits" at end
	log.Printf("constraintSetNFlag:%d", constraintSetNFlag)
	levelIdc := bv.getBits(8)
	log.Printf("levelIdc:%d", levelIdc)
	seqParameterSetID := bv.getExpGolomb()
	log.Printf("seqParameterSetID:%d", seqParameterSetID)
	if profileIdc == 100 ||
		profileIdc == 110 ||
		profileIdc == 122 ||
		profileIdc == 244 ||
		...
		if chromaFormatIdc == 3 {
			...
			log.Printf("eparateColourPlaneFlag:%v", eparateColourPlaneFlag)
		}
		...
		log.Printf("seqScalingMatrixPresentFlag:%d", seqScalingMatrixPresentFlag)
	...
	log.Printf("log2MaxFrameNumMinus4:%d", log2MaxFrameNumMinus4)
	p.log2MaxFrameNum = log2MaxFrameNumMinus4 + 4
	picOrderCntType := bv.getExpGolomb()
	log.Printf("picOrderCntType:%d", picOrderCntType)
	if picOrderCntType == 0 {
		log2MaxPicOrderCntLsbMinus4 := bv.getExpGolomb()
		log.Printf("log2MaxPicOrderCntLsbMinus4:%d", log2MaxPicOrderCntLsbMinus4)
	...
	log.Printf("maxNumRefFrames:%d", maxNumRefFrames)
	gapsInFrameNumValueAllowedFlag := bv.get1Bit()
	log.Printf("gapsInFrameNumValueAllowedFlag:%d", gapsInFrameNumValueAllowedFlag)
	picWidthInMbsMinus1 := bv.getExpGolomb()
	log.Printf("picWidthInMbsMinus1:%d", picWidthInMbsMinus1)
	picHeightInMapUnitsMinus1 := bv.getExpGolomb()
	log.Printf("picHeightInMapUnitsMinus1:%d", picHeightInMapUnitsMinus1)
	frameMbsOnlyFlag := bv.get1BitBoolean()
	log.Printf("frameMbsOnlyFlag:%v", frameMbsOnlyFlag)
	if !frameMbsOnlyFlag {
		bv.skipBits(1) // mb_adaptive_frame_field_flag
	}
	bv.skipBits(1) // direct_8x8_inference_flag
	frameCroppingFlag := bv.get1Bit()
	log.Printf("frameCroppingFlag:%d", frameCroppingFlag)
	...
	log.Printf("vuiParametersPresentFlag:%d", vuiParametersPresentFlag)
