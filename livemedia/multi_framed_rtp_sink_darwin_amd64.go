...
func (s *MultiFramedRTPSink) sendPacketIfNecessary() {
	...
	if s.noFramesLeft {
		...
		uSecondsToGo := secsDiff*1000000 + (int64(s.nextSendTime.Usec) - int64(timeNow.Usec))
		if uSecondsToGo < 0 || secsDiff < 0 { // sanity check: Make sure that the time-to-delay is non-negative:
			uSecondsToGo = 0
		}

		// Delay this amount of time:
		//log.Printf("[MultiFramedRTPSink::sendPacketIfNecessary] uSecondsToGo: %d", uSecondsToGo)
